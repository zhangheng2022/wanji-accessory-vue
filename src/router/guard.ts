...
export function registerNavigationGuard(router: Router) {
  ...
  router.beforeEach(async (to, _from) => {
    ...
    if (!getToken()) {
      ...
      // 如果没有token,则在url上获取token
      if (to.query.token) {
        console.log("获取到token", to.query.token)
        // 设置token
    ...
    try {
      ...
      // 生成可访问的 Routes
      routerConfig.dynamic ? permissionStore.setRoutes(roles, userStore.dynamicMenus) : permissionStore.setAllRoutes()
